 ...
 * 
 */
class CDVZ_carousel extends Codevz_Plus {
	...
	public function out( $atts, $content = '' ) {
		...
		$slick = array(
			...
			'responsive'		=> array(
  				array(
					'breakpoint'	=> (int) Codevz_Plus::option( 'responsive_2', 768 ),
					'settings'		=> array(
  				...
  				array(
					'breakpoint'	=> (int) Codevz_Plus::option( 'responsive_3', 480 ),
					'settings'		=> array(
		...
		if ( isset( $public ) || Codevz_Plus::$vc_editable || Codevz_Plus::$is_admin ) {
			...
			$css_array = array(
				...
				( $atts['centermode'] ? 'sk_slides' : 'x') =>  $css_id . ' .slick-slide',
				( $atts['centermode'] ? 'sk_center' : 'x') =>  $css_id . ' .slick-center',
			);
			...
			$css .= $atts['gap'] ? $css_id . ' .slick-slide { margin: 0 ' . $atts['gap'] . ' }' : '';
		}
		...
		if ( $atts['sync'] ) {
			...
			$sync = 'is_synced';
			$sync_data = ' data-sync="' . $atts['sync'] . '" data-class="' . $atts['class'] . '"';
		}
		...
		// Out
		$out = '<div id="' . $atts['id'] . '"' . Codevz_Plus::classes( $atts, $classes ) . $sync_data . ' data-slick=\'' . json_encode( $slick ) . '\'>' . do_shortcode( $content ) . '</div><div' . Codevz_Plus::data_stlye( $css, $css_t, $css_m ) . '></div>';
