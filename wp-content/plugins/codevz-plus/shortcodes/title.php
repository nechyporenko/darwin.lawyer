 ...
 * 
 */
class CDVZ_title extends Codevz_Plus {
		...
		Codevz_Plus::vc_map( array(
			...
			'params'		=> array(
				...
				array(
					...
					"heading"     	=> esc_html__( "Icon after styling", 'codevz'),
				...
				array(
					...
					"heading"     	=> esc_html__( "Shape styling", 'codevz'),
				...
				array(
					...
					'hover_id'	 	=> 'sk_shape_hover',
					"heading"     	=> esc_html__( "Shape 2 styling", 'codevz'),
	...
	public function out( $atts, $content = '<h3>Title Element</h3>' ) {
		...
		if ( $atts['bline'] ) {
			...
			$bline_css = ( $atts['css_right_line_left'] && $atts['bline'] === 'cz_line_both_side' ) ? ' style="' . ( self::$is_rtl ? 'right' : 'left' ) . ':' . $atts['css_right_line_left'] . '"' : '';
		...
		if ( isset( $public ) || Codevz_Plus::$vc_editable || Codevz_Plus::$is_admin ) {
			...
			$css_array = array(
				...
				'sk_overall_hover' 		=> $css_id . ':hover .cz_title_content',
		...
		// Animation
		$animation = '';
		if ( $atts['css_animation'] && $atts['css_animation'] !== 'none' ) {
		...
		$out .= '<div id="' . $atts['id'] . '"' . Codevz_Plus::classes( $atts, $classes ) . Codevz_Plus::data_stlye( $css,  $css_t, $css_m ) . '>' . $shape_out . '<div class="cz_title_content">' . $line_before . $icon . '<div class="cz_wpe_content">' . ( function_exists( 'wpb_js_remove_wpautop' ) ? wpb_js_remove_wpautop( $content, true ) : do_shortcode( $content ) ) . '</div>' . $icon_after . $line_after. '</div></div>';
		$out .= $animation ? '</div>' : '';
